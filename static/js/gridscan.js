// GridScan Vanilla JS Implementation
// Expects THREE and POSTPROCESSING to be available globally
...
window.initGridScan = function(containerId) {
    ...
    // Configuration
    const sensitivity = 0.15; // Lowered from 0.55 for much more subtle movement
    const lineThickness = 1;
    const linesColor = '#342b45';
    const gridScale = 0.12;
    const scanColor = '#b832f4';
    const scanOpacity = 0.4;
    const enablePost = true;
    const bloomIntensity = 0.6;
    const chromaticAberration = 0.0025;
    const noiseIntensity = 0.02;
    const lineJitter = 0.22;
    const scanSoftness = 2.2;
    const scanGlow = 0.5;
    ...
    if (enablePost) {
        ...
        const chroma = new POSTPROCESSING.ChromaticAberrationEffect({
    ...
    const tiltTarget = { current: 0 };
    const yawTarget = { current: 0 };

    const lookCurrent = new THREE.Vector2(0, 0);
    const lookVel = new THREE.Vector2(0, 0);
    const tiltCurrent = { current: 0 };
    const tiltVel = { current: 0 };
    const yawCurrent = { current: 0 };
    const yawVel = { current: 0 };
    ...
    const onResize = () => {
        renderer.setSize(window.innerWidth, window.innerHeight);
        material.uniforms.iResolution.value.set(window.innerWidth, window.innerHeight, renderer.getPixelRatio());
        if (composer) composer.setSize(window.innerWidth, window.innerHeight);
    };
    window.addEventListener('resize', onResize);

    const s = THREE.MathUtils.clamp(sensitivity, 0, 1);
    // Reduced the lerp ranges to make the tilt/skew much more subtle
    const skewScale = THREE.MathUtils.lerp(0.02, 0.08, s);
    const tiltScale = THREE.MathUtils.lerp(0.04, 0.15, s);
    const yawScale = THREE.MathUtils.lerp(0.03, 0.12, s);
