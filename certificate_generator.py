...
def draw_ribbon_banner(draw, cx, cy, width, height, bg_color=(20, 48, 100), border_color=(212, 175, 55)):
    ...
    # Left ribbon tail
    tail_left = [(x1 - notch, y1 + 10), (x1, y1), (x1, y2), (x1 - notch, y2 - 10), (x1 - notch + 16, cy)]
    draw.polygon(tail_left, fill=(12, 28, 60), outline=border_color)
    
    # Right ribbon tail
    tail_right = [(x2 + notch, y1 + 10), (x2, y1), (x2, y2), (x2 + notch, y2 - 10), (x2 + notch - 16, cy)]
    ...
    draw.rectangle([x1 + 4, y1 + 4, x2 - 4, y2 - 4], outline=(254, 240, 138), width=1)
...
def draw_award_emblem(draw, cx, cy):
    ...
    draw.polygon([(cx - 24, cy + 30), (cx - 44, cy + 115), (cx - 24, cy + 100), (cx - 4, cy + 115)], fill=(180, 83, 9), outline=(212, 175, 55))
    draw.polygon([(cx + 4, cy + 30), (cx + 24, cy + 115), (cx + 44, cy + 100), (cx + 64, cy + 115)], fill=(217, 119, 6), outline=(212, 175, 55))
    ...
    # Outer gold circle
    draw.ellipse([cx - 52, cy - 52, cx + 52, cy + 52], fill=(254, 240, 138), outline=(180, 83, 9), width=3)
    # Inner navy ring
    draw.ellipse([cx - 42, cy - 42, cx + 42, cy + 42], fill=(15, 23, 42), outline=(212, 175, 55), width=2)
    ...
    draw.text((cx, cy - 14), "★  ★  ★", fill=(253, 224, 71), font=f_star, anchor="mm")
    draw.text((cx, cy + 2), "AWARD", fill=(255, 255, 255), font=f_award, anchor="mm")
    draw.text((cx, cy + 18), "2026-27", fill=(254, 240, 138), font=f_year, anchor="mm")
...
def draw_student_silhouettes(draw, x_start, y_start):
    ...
    accent_dots = [(225, 29, 72), (37, 99, 235), (217, 119, 6), (124, 58, 237), (13, 148, 136)]
    for j in range(36):
    ...
    draw.polygon([(f1_x - 12, f1_y + 12), (f1_x + 16, f1_y - 28), (f1_x + 30, f1_y - 22), (f1_x + 4, f1_y + 18)], fill=c_slate)
    draw.polygon([(f1_x + 20, f1_y - 22), (f1_x + 50, f1_y - 12), (f1_x + 46, f1_y - 4), (f1_x + 16, f1_y - 16)], fill=c_slate)
    draw.polygon([(f1_x - 2, f1_y - 14), (f1_x - 26, f1_y + 10), (f1_x - 22, f1_y + 14), (f1_x + 2, f1_y - 6)], fill=c_slate)
    draw.polygon([(f1_x + 2, f1_y + 14), (f1_x + 36, f1_y + 52), (f1_x + 28, f1_y + 56), (f1_x - 4, f1_y + 18)], fill=c_slate)
    draw.polygon([(f1_x - 10, f1_y + 12), (f1_x - 34, f1_y + 58), (f1_x - 26, f1_y + 60), (f1_x - 2, f1_y + 16)], fill=c_slate)

    # Pose 2: Football / Sports Athlete with Ball (Sports)
    f2_x = x_start + 145
    f2_y = y_start + 55
    draw.ellipse([f2_x - 14, f2_y - 56, f2_x + 14, f2_y - 28], fill=c_navy)
    draw.polygon([(f2_x - 20, f2_y + 24), (f2_x - 16, f2_y - 26), (f2_x + 14, f2_y - 26), (f2_x + 20, f2_y + 24)], fill=c_navy)
    draw.polygon([(f2_x + 12, f2_y + 16), (f2_x + 48, f2_y + 36), (f2_x + 44, f2_y + 44), (f2_x + 6, f2_y + 26)], fill=c_navy)
    # Sports ball in flight
    draw.ellipse([f2_x + 56, f2_y + 10, f2_x + 82, f2_y + 36], fill=(255, 255, 255), outline=c_navy, width=2)
    draw.polygon([(f2_x + 65, f2_y + 19), (f2_x + 72, f2_y + 19), (f2_x + 76, f2_y + 26), (f2_x + 69, f2_y + 31), (f2_x + 62, f2_y + 26)], fill=c_navy)
    ...
    draw.polygon([(f3_x - 22, f3_y + 80), (f3_x - 18, f3_y - 24), (f3_x + 18, f3_y - 24), (f3_x + 22, f3_y + 80)], fill=c_navy)
    draw.polygon([(f3_x - 16, f3_y - 20), (f3_x - 38, f3_y - 72), (f3_x - 28, f3_y - 76), (f3_x - 6, f3_y - 22)], fill=c_navy)
    # Golden trophy
    tx, ty = f3_x - 42, f3_y - 102
    draw.polygon([(tx - 13, ty), (tx + 13, ty), (tx + 7, ty + 22), (tx - 7, ty + 22)], fill=c_gold)
    ...
    draw.polygon([(f4_x - 18, f4_y + 40), (f4_x - 14, f4_y - 24), (f4_x + 14, f4_y - 24), (f4_x + 18, f4_y + 40)], fill=c_blue)
    draw.polygon([(f4_x + 12, f4_y - 20), (f4_x + 40, f4_y - 56), (f4_x + 45, f4_y - 48), (f4_x + 16, f4_y - 14)], fill=c_blue)
    draw.polygon([(f4_x - 12, f4_y - 20), (f4_x - 38, f4_y - 52), (f4_x - 34, f4_y - 58), (f4_x - 6, f4_y - 16)], fill=c_blue)
    ...
    draw.polygon([(f5_x - 18, f5_y + 45), (f5_x - 16, f5_y - 20), (f5_x + 16, f5_y - 20), (f5_x + 18, f5_y + 45)], fill=c_slate)
    draw.polygon([(f5_x - 25, f5_y + 10), (f5_x + 5, f5_y + 10), (f5_x + 9, f5_y + 24), (f5_x - 20, f5_y + 24)], fill=(59, 130, 246))
    draw.polygon([(f5_x - 20, f5_y - 4), (f5_x - 20, f5_y + 10), (f5_x - 9, f5_y + 10), (f5_x - 9, f5_y - 4)], fill=(147, 197, 253))

def generate_certificate(participant_name, event_name, event_date, college_name=None, cert_code=None, rank="1st", department="BCA", semester="5th Sem", team_name=None):
    """
    ...
    for r_step in range(40, 0, -2):
        ...
        draw.rectangle([(r_step, r_step), (width - r_step, height - r_step)], outline=(shade, shade, shade))
    ...
    draw.rectangle([(28, 28), (width - 28, height - 28)], outline=border_navy, width=16)
    
    # Inner Rich Gold Border
    draw.rectangle([(52, 52), (width - 52, height - 52)], outline=border_gold, width=6)
    
    # Fine Inner Slate Guideline
    draw.rectangle([(66, 66), (width - 66, height - 66)], outline=slate_border, width=2)
    
    # Inner Fine Gold Guideline
    draw.rectangle([(74, 74), (width - 74, height - 74)], outline=(243, 210, 102), width=1)
    ...
    for cx_c, cy_c in corners:
        draw.rectangle([(cx_c, cy_c), (cx_c + corner_size, cy_c + corner_size)], fill=(254, 240, 138), outline=border_gold, width=2)
        mid_x, mid_y = cx_c + corner_size // 2, cy_c + corner_size // 2
        draw.polygon([(mid_x, cy_c + 6), (cx_c + corner_size - 6, mid_y), (mid_x, cy_c + corner_size - 6), (cx_c + 6, mid_y)], fill=border_navy)
        draw.ellipse([mid_x - 5, mid_y - 5, mid_x + 5, mid_y + 5], fill=gold_accent)
    ...
    draw.text((width // 2, 130), inst_name, fill=border_navy, font=f_inst, anchor="mm")
    ...
    draw.text((width // 2, 195), sub1_text, fill=(51, 65, 85), font=f_sub1, anchor="mm")
    ...
    draw.text((width // 2, 238), addr_text, fill=(100, 116, 139), font=f_addr, anchor="mm")

    # Decorative Divider Line with Center Diamond
    divider_y = 278
    draw.line([(width // 2 - 380, divider_y), (width // 2 + 380, divider_y)], fill=border_gold, width=3)
    draw.polygon([
        (width // 2, divider_y - 9), 
    ...
    banner_cx = width // 2
    banner_cy = 345
    draw_ribbon_banner(draw, banner_cx, banner_cy, banner_w, banner_h, bg_color=(20, 48, 100), border_color=border_gold)
    
    # Bold Kannada Text inside Banner
    f_kannada = get_font_by_name("Nirmala.ttc", 38, index=1)
    kannada_title = "ಸಾಂಸ್ಕೃತಿಕ 2026-27"
    draw.text((banner_cx, banner_cy - 2), kannada_title, fill=(255, 255, 255), font=f_kannada, anchor="mm")
    ...
    draw.text((width // 2 + 2, 442), title_text, fill=(254, 202, 202), font=f_title, anchor="mm")
    draw.text((width // 2, 440), title_text, fill=(185, 28, 28), font=f_title, anchor="mm") # Elegant Red

    # Gold decorative flourish line beneath title
    draw.line([(width // 2 - 240, 484), (width // 2 + 240, 484)], fill=border_gold, width=2)
    draw.ellipse([width // 2 - 6, 484 - 6, width // 2 + 6, 484 + 6], fill=border_gold)
    ...
    # Clean semester
    sem_text = semester.strip() if semester else "5th Sem"
    ...
    draw.text((name_x + (fill_name_w - w_l1_name) // 2 - 15, y_l1 - 10), p_name, fill=blue_ink, font=f_handwriting_name)
    draw.line([(name_x, y_l1 + 45), (name_x + fill_name_w, y_l1 + 45)], fill=(148, 163, 184), width=2)
    ...
    # Secured fill-in
    draw.text((cur_x2 + (fill1_w - w_sec) // 2, y_l2 - 6), sec_details, fill=blue_ink, font=f_handwriting)
    draw.line([(cur_x2 + 4, y_l2 + 40), (cur_x2 + fill1_w - 4, y_l2 + 40)], fill=(148, 163, 184), width=2)
    cur_x2 += fill1_w
    ...
    # Semester fill-in
    draw.text((cur_x2 + (fill2_w - w_sem) // 2, y_l2 - 6), sem_text, fill=blue_ink, font=f_handwriting)
    draw.line([(cur_x2 + 4, y_l2 + 40), (cur_x2 + fill2_w - 4, y_l2 + 40)], fill=(148, 163, 184), width=2)
    cur_x2 += fill2_w
    ...
    # Place fill-in
    draw.text((cur_x3 + (fill3_w - w_place) // 2, y_l3 - 6), place_text, fill=blue_ink, font=f_handwriting)
    draw.line([(cur_x3 + 4, y_l3 + 40), (cur_x3 + fill3_w - 4, y_l3 + 40)], fill=(148, 163, 184), width=2)
    cur_x3 += fill3_w
    ...
    # Event fill-in
    draw.text((cur_x3 + (fill4_w - w_ev) // 2, y_l3 - 6), l3_ev, fill=blue_ink, font=f_handwriting)
    draw.line([(cur_x3 + 4, y_l3 + 40), (cur_x3 + fill4_w - 4, y_l3 + 40)], fill=(148, 163, 184), width=2)
    cur_x3 += fill4_w
    ...
    if team_name:
        ...
        draw.text((width // 2, y_team), team_display, fill=(71, 85, 105), font=get_font_by_name("georgiai.ttf", 28), anchor="mm")
    ...
    sig1_x = 640
    draw.line([(sig1_x - 140, sig_y), (sig1_x + 140, sig_y)], fill=(148, 163, 184), width=2)
    draw.text((sig1_x, sig_y - 45), "Prof. Chethana", fill=(30, 58, 138), font=f_sig_script, anchor="mm")
    draw.text((sig1_x, sig_y + 30), "hod", fill=border_navy, font=f_sig_label, anchor="mm")
    draw.text((sig1_x, sig_y + 60), "Head of Department", fill=(100, 116, 139), font=f_sig_sub, anchor="mm")

    # Signature 2: Lectures / Faculty Coordinator (Center)
    sig2_x = 1120
    draw.line([(sig2_x - 140, sig_y), (sig2_x + 140, sig_y)], fill=(148, 163, 184), width=2)
    draw.text((sig2_x, sig_y - 45), "K. Srinivas Rao", fill=(30, 58, 138), font=f_sig_script, anchor="mm")
    draw.text((sig2_x, sig_y + 30), "lectures", fill=border_navy, font=f_sig_label, anchor="mm")
    draw.text((sig2_x, sig_y + 60), "Faculty Coordinator", fill=(100, 116, 139), font=f_sig_sub, anchor="mm")

    # Signature 3: Principal (Right-Center)
    sig3_x = 1600
    draw.line([(sig3_x - 140, sig_y), (sig3_x + 140, sig_y)], fill=(148, 163, 184), width=2)
    draw.text((sig3_x, sig_y - 45), "Dr. Ramesh Hegde", fill=(30, 58, 138), font=f_sig_script, anchor="mm")
    draw.text((sig3_x, sig_y + 30), "Principal", fill=border_navy, font=f_sig_label, anchor="mm")
    draw.text((sig3_x, sig_y + 60), "Shree Daksha Academy", fill=(100, 116, 139), font=f_sig_sub, anchor="mm")

    # 11. Security QR Code & Verification (Bottom-Left Corner)
    verify_url = f"{Config.BASE_URL}/verify?id={cert_code}"
    qr = qrcode.QRCode(
        version=1,
    ...
    # Paste QR Code on Bottom Left with safe margins
    img.paste(qr_img, (115, height - 245))
    
    # Verification details text
    f_v1 = get_font_by_name("arial.ttf", 13)
    f_v2 = get_font_by_name("arialbd.ttf", 14)
    draw.text((215, height - 235), "Verify Certificate:", fill=(100, 116, 139), font=f_v1)
    draw.text((215, height - 212), f"ID: {cert_code}", fill=border_navy, font=f_v2)
    draw.text((215, height - 190), f"Date: {event_date}", fill=(100, 116, 139), font=f_v1)
